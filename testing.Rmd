...
```{r}
# Extract Saturdays and Sundays
weekends <- filter(mergeweek, Weekday %in% c("Saturday", "Sunday"))
...
p <- mergeweek %>%
  ...
  geom_point() +
  theme_minimal()
# Add vertical lines for Saturdays and Sundays
p + geom_vline(data = weekends, aes(xintercept = as.numeric(Date)), color = "lightblue", alpha = 0.5)
```
